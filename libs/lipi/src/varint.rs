...
mod tests {
    ...
    fn test_varint_decoding() {
        ...
        for exp in (14..=63).step_by(7) {
            let groups = (exp / 7) as usize;

            let mut minus_one = vec![0xFF; groups - 1];
            minus_one.push(0x7F);
            check(2u64.pow(exp) - 1, &minus_one);

            let mut exact = vec![0x80; groups];
            exact.push(0x01);
            check(2u64.pow(exp), &exact);
        }
